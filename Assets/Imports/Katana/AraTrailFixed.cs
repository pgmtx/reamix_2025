...
namespace Ara{
    ...
    [ExecuteInEditMode]
    public class AraTrailFixed : MonoBehaviour {
            ...
            if (trail.Count > 1)
            {
                ...
                for (int i = trail.Count - 1; i >= 0; --i)
                {
                    ...
                    if (alignment == TrailAlignment.Velocity)
                    {
                        ...
                        // Fix flip au changement de direction : on force le bitangent
                        // a rester coherent avec Vector3.up pour eviter le retournement
                        if (Vector3.Dot(bitangent, Vector3.up) < 0)
                            bitangent = -bitangent;
                    }
                    else
                    {
